class ShipsController < ApplicationController
  ...
  def index
    ...
    @locations = Location.order('ship_id, id ASC')
    @destinations = Destination.order('ship_id, id ASC')

    @ships = Ship.order('id ASC')
    # respond_to do |format| 
    #   format.json { render json: @ships.includes(:locations, :destinations)}
    #   format.html
    # end
    ###############THE BIG ACHE EFFORT###############
    # @ships = Ship.order('id ASC').includes(:locations, :destinations)
